...
int DPTriggerAnalyzer::process_event(PHCompositeNode* topNode) {
  ...
  //  std::cout << "Entering DPTriggerAnalyzer::process_event: " << event_id << std::endl;
  ...
  int Nhits_YNIM_1XT=0;
  int Nhits_YNIM_2XT=0;
  int Nhits_YNIM_3XT=0;
  int Nhits_YNIM_4XT=0;
  int Nhits_YNIM_1XB=0;
  int Nhits_YNIM_2XB=0;
  int Nhits_YNIM_3XB=0;
  int Nhits_YNIM_4XB=0;
  int Nhits_YNIM_1YL=0;
  int Nhits_YNIM_2YL=0;
  int Nhits_YNIM_4Y1L=0;
  int Nhits_YNIM_4Y2L=0;
  int Nhits_YNIM_1YR=0;
  int Nhits_YNIM_2YR=0;
  int Nhits_YNIM_4Y1R=0;
  int Nhits_YNIM_4Y2R=0;
  ...
  for (Int_t ihit = 0; ihit < _hit_vector->size(); ++ihit) {
    ...
    if(geomSvc->getTriggerLv(hit->get_detector_id())<0) continue;
    
    if(Verbosity() >= Fun4AllBase::VERBOSITY_A_LOT) {
      std::cout
        << " detector_id: " << hit->get_detector_id()
        << " TriggerLv: " << geomSvc->getTriggerLv(hit->get_detector_id())
        << std::endl;
    }
  ...
  bool HXB=(Nhits_YNIM_1XB>0 and Nhits_YNIM_2XB>0 and Nhits_YNIM_3XB>0 and Nhits_YNIM_4XB>0);
  bool HXT=(Nhits_YNIM_1XT>0 and Nhits_YNIM_2XT>0 and Nhits_YNIM_3XT>0 and Nhits_YNIM_4XT>0);
  bool HYL=(Nhits_YNIM_1YL>0 and Nhits_YNIM_2YL>0 and Nhits_YNIM_4Y1L>0 and Nhits_YNIM_4Y2L>0);
  bool HYR=(Nhits_YNIM_1YR>0 and Nhits_YNIM_2YR>0 and Nhits_YNIM_4Y1R>0 and Nhits_YNIM_4Y2R>0);
  bool nim1_on = HYL or HYR;
  bool nim2_on = HXT or HXB;
  if(nim1_on) { if (! _event_header->get_trigger(SQEvent::NIM1)) cout << "NIM1 true vs false." << endl; }
  else        { if (  _event_header->get_trigger(SQEvent::NIM1)) cout << "NIM1 false vs true." << endl; }
  if(nim2_on) { if (! _event_header->get_trigger(SQEvent::NIM2)) cout << "NIM2 true vs false." << endl; }
  else        { if (  _event_header->get_trigger(SQEvent::NIM2)) cout << "NIM2 false vs true." << endl; }
  //cout << "NIM1 " << nim1_on << " " << _event_header->get_trigger(SQEvent::NIM1) << endl;
  //cout << "NIM2 " << nim2_on << " " << _event_header->get_trigger(SQEvent::NIM2) << endl;
  ...
  if(fpga1_on) { if (! _event_header->get_trigger(SQEvent::MATRIX1)) cout << "FPGA1 true vs false." << endl; }
  else         { if (  _event_header->get_trigger(SQEvent::MATRIX1)) cout << "FPGA1 false vs true." << endl; }
  if(fpga2_on) { if (! _event_header->get_trigger(SQEvent::MATRIX2)) cout << "FPGA2 true vs false." << endl; }
  else         { if (  _event_header->get_trigger(SQEvent::MATRIX2)) cout << "FPGA2 false vs true." << endl; }
  if(fpga3_on) { if (! _event_header->get_trigger(SQEvent::MATRIX3)) cout << "FPGA3 true vs false." << endl; }
  else         { if (  _event_header->get_trigger(SQEvent::MATRIX3)) cout << "FPGA3 false vs true." << endl; }
  if(fpga4_on) { if (! _event_header->get_trigger(SQEvent::MATRIX4)) cout << "FPGA4 true vs false." << endl; }
  else         { if (  _event_header->get_trigger(SQEvent::MATRIX4)) cout << "FPGA4 false vs true." << endl; }
  if(fpga5_on) { if (! _event_header->get_trigger(SQEvent::MATRIX5)) cout << "FPGA5 true vs false." << endl; }
  else         { if (  _event_header->get_trigger(SQEvent::MATRIX5)) cout << "FPGA5 false vs true." << endl; }
  //cout << "FPGA1 " << fpga1_on << " " << _event_header->get_trigger(SQEvent::MATRIX1) << endl;
...
bool DPTriggerAnalyzer::buildHitPattern(int nTrHits, int uniqueTrIDs[])
{
  ...
  for(int i = 0; i < nTrHits; ++i){
    ...
    int index = GeomSvc::instance()->getTriggerLv(detectorID);
