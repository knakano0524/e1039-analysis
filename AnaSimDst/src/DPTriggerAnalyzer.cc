...
int DPTriggerAnalyzer::process_event(PHCompositeNode* topNode) {
  ...
  for(Int_t ihit = 0; ihit < _hit_vector->size(); ++ihit) {
    ...
    int index = GetHodoStation(hit->get_detector_id()) - 1;
...
short DPTriggerAnalyzer::GetHodoStation(const int det_id)
{
  string name = GeomSvc::instance()->getDetectorName(det_id);
  if (name.size() > 0 && name[0] == 'H') return name[1] - '0';
  return 0;
}
