...
//#include "G4_1039InsensDetector.C"
#include "G4_Target.C"
...
R__LOAD_LIBRARY(libabisimulation)
//R__LOAD_LIBRARY(libE906LegacyGen)
R__LOAD_LIBRARY(libE1039PhysicsGen)
R__LOAD_LIBRARY(libphg4hit)
    ...
    )
{
  ...
  if(gen_pythia8) {
    ...
    //Vertex study===
    pythia8->set_vertex_distribution_function(PHHepMCGenHelper::Gaus, PHHepMCGenHelper::Gaus, PHHepMCGenHelper::Gaus, PHHepMCGenHelper::Uniform );
    pythia8->set_vertex_distribution_mean(0.0, 0.0, target_coil_pos_z, 0.0);
    pythia8->set_vertex_distribution_width(target_z/2., target_z/2., target_l/2., 0.0);
    // pythia8->set_vertex_size_function(PHG4SimpleEventGenerator::Uniform);
    // pythia8->set_vertex_size_parameters(0.0, 0.0);
  ...
  SetupSensitiveDetectors(g4Reco, 0);
    ...
    //insensitive elements of the spectrometer
  //gROOT->LoadMacro("G4_1039InsensDetector.C");
  //SetupInSensitiveDetectors(g4Reco,0);
  ...
  // const bool abisim_ana = true;


  //Abinash Pun....  
  abisimulation *abisim1 = new abisimulation();
  se->registerSubsystem(abisim1);
 

  // // E906 legacy generator..
  // E906LegacyGen *abi_e906legacy = new  E906LegacyGen(abi_rootfile);
  // //E906LegacyGen *abi_e906legacy = new  E906LegacyGen();
  // se->registerSubsystem(abi_e906legacy);
